...
# Лиза по умолчанию — 110 минут (было 45). Раскрыто вживую 02.09.2026: этот
# же прогон, где подняли tools/delegate_policy.json timeout_seconds до 5400с
# (90 мин, см. commit 8cb27ce — эмпирика по historical held_sec, p95≈64мин,
# max≈83мин), вызвал ОДИН И ТОТ ЖЕ delegate-звонок, который легитимно шёл
# дольше старой 45-минутной лизы. delegate_invoke.py's мост НЕ шлёт
# delegation_heartbeat во время единственного блокирующего await(request) —
# значит лиза не продлевается сама по себе, пока делегат реально работает.
# expire_stale() (см. ниже) НЕ защищает истечение лизы механизмом
# termination_unconfirmed — по документированному дизайну (см.
# docs/agent-safety-architecture.md, §«Делегат завис») голая lease_expired
# считается «признаков жизни нет» и разрешает оркестратору переделегировать
# задачу. Это было безопасно, пока лиза (45 мин) была намного больше любого
# реального вызова (~12 мин), но перестало быть безопасным допущением, когда
# сам timeout_seconds подняли до 90 мин — окно в 45 минут стало УЖЕ, чем
# легитимная работа, и рисковало создать двойное делегирование на живую,
# ещё работающую задачу (класс инцидента 31.08). Лиза теперь заведомо больше
# tools/delegate_policy.json's timeout_seconds (5400с) с запасом на
# post-completion housekeeping (merge/close/телеметрия) — так что настоящий
# зависший делегат по-прежнему ловится (просто чуть позже), а легитимно
# долгая работа больше не гонит ложный lease_expired посреди реального
# успеха. Структурный вопрос "должна ли голая lease_expired тоже получать
# termination_unconfirmed-защиту, как mcp_transport_timeout" — сознательно
# НЕ решён этим коммитом, это отдельное архитектурное решение.
DEFAULT_LEASE_SEC = 110 * 60
