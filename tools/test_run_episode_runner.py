...
# The production Codex/Claude lifecycle was moved behind a tiny dispatcher.
# These established regression tests intentionally keep exercising that legacy
# lifecycle, not the dispatcher itself.
RUN_EPISODE = Path(__file__).with_name("run_episode_legacy.sh")
...
def _tts_quota_gate(source: str) -> str:
    ...
    end = source.index("CODEX_PROMPT_FILE=", start)
    return source[start:end]
...
def test_codex_doctor_is_inside_codex_runner_gate_only():
    ...
    end = source.index("# Gemini TTS free-tier daily quota preflight", start)
...
def test_success_without_episode_is_pipeline_incomplete(tmp_path: Path):
    result = _run_completion_gate(tmp_path, code=0, episode_exists=False, validate_code=0)
...
def test_success_with_invalid_episode_is_pipeline_incomplete(tmp_path: Path):
    result = _run_completion_gate(tmp_path, code=0, episode_exists=True, validate_code=1)
    assert result == ("failed", "semantic_failure", "pipeline_incomplete")
    assert (tmp_path / "validate-called").exists()


def test_success_with_valid_episode_keeps_success_classification(tmp_path: Path):
    result = _run_completion_gate(tmp_path, code=0, episode_exists=True, validate_code=0)
...
def test_success_with_valid_episode_but_no_publication_is_pipeline_incomplete(tmp_path: Path):
    result = _run_completion_gate(
        tmp_path, code=0, episode_exists=True, validate_code=0, publication_created=False
    )
    assert result == ("failed", "semantic_failure", "pipeline_incomplete")
    assert (tmp_path / "validate-called").exists()
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("MCP tool call requires approval", "control_plane_failure", "mcp_invocation_invalid"),
        ("", "semantic_failure", ""),
    ],
)
def test_nonzero_exit_classification_is_unchanged(
    ...
    result = subprocess.run(
        ["bash", "-c", shell], cwd=tmp_path, check=True, capture_output=True, text=True
    )
