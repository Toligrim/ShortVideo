...
2. Сценарист → режиссёр → озвучка → критик → рендер — все шаги скилла produce,
   ...
   `codex` (инструмент codex/codex-reply); их результат проверяется дальше по
   конвейеру (валидация схемы, критик). Skill ~/.codex/skills/delegate-with-
   opencode и бинарь opencode-tool ЗАПРЕЩЕНЫ технически (бинарь подменён
   отказывающим шимом на время этого прогона) и по правилам — не пытайся их
   обойти. Прецедент auto-20260831-050001 (31.08.2026): делегат через
   opencode-tool не смог прочитать словарь транслитерации вне песочницы
   проекта, и в попытке обойти это ты сам явно велел оставить буквальный
   плейсхолдер {{SHOW|термин}} вместо разметки {{биткоин|биткоин}} —
   он дошёл до эфира и увидел его сам оператор. Если делегату нужен файл вне
   /home/toligrim/projects/ShortVideo (например ~/.claude/commands/tts.md),
   вставь его содержимое в текст задачи целиком, а не проси делегата читать
   путь снаружи проекта.
   НЕ делегируй один и тот же шаг (сценарист/режиссёр) на один и тот же slug
   ПОВТОРНО, пока не убедился, что предыдущий вызов реально завершился или
   безвозвратно умер — если результат неясен, дождись его (codex-reply /
   опрос статуса), не запускай второй параллельный вызов «на всякий случай».
   Прецедент auto-20260831-164055 (31.08.2026): оркестратор трижды
   параллельно делегировал сценариста на один slug; третий делегат, увидев
   «грязное» рабочее дерево, сам без всякой инструкции выполнил
   `git stash push --include-untracked` (снёс ВСЕ незакоммиченные файлы
   репозитория, включая tools/pipeline_log.py, от которого зависит весь
   конвейер), не восстановил стеш и, наткнувшись на гонку с другим
   делегатом за тот же файл, начал убивать процессы (`kill`/`pkill` по PID
   из `ps`) — убил родительский orchestrator целиком. Разбор —
   ...
   Если делегат обнаруживает гонку/конфликт (целевой файл уже существует
   и отличается от ожидаемого) — он останавливается и сообщает об этом в
   финальном ответе, а не борется за файл удалением/перезаписью/убийством
   процессов.
