...
1. ВЫБЕРИ ТЕМУ САМ. Критерий №1 — узнаваемость, а не узость: тема должна быть на
   ...
   ГЛУБИНА МЕХАНИЗМА — жёсткий отсев. Если весь ответ на вопрос ролика
   сводится к «для этого есть отдельный чип / отдельная подсистема / отдельный
   сервер» — тема НЕ годится: на 60 секунд там нет глубины, показывать нечего
   кроме коробки с ярлыком. Годится тема, где есть конкретный шаг, трюк или
   структура данных, который можно показать буквально и который зрителя
   удивляет («число переполнилось при конверсии», «индекс перевёрнут заранее»,
   «ключи расходуются по одному и сдвигаются»). Прецедент отсева —
   auto-20260909-052001 «почему будильник звонит в авиарежиме»: весь механизм
   был «в телефоне есть отдельные часы RTC и аппаратный alarm», глубины на
   ролик не хватило, вышел слоп.

   РАМКА-КЛИШЕ — ротация, не только фактовый дедуп. Список выше ловит повтор
   ФАКТА. Отдельно следи за повтором РАМКИ подачи: «работает без интернета /
   всё ещё работает, когда убрали X / почему Y не ломается, когда рвётся Z /
   почему A не боится B» — если последние 3–4 ролика уже эксплуатировали эту
   рамку, возьми другую, даже если факт свежий. «Будильник в авиарежиме» была
   пятой вариацией «работает офлайн» после TOTP-кода, хотворда «Окей Гугл»,
   GPS и почты — рамка приелась.

{past_titles_block}

2. Сценарист → критик (сценарий) → режиссёр → озвучка → рендер — все шаги
   скилла produce, включая проверки и право вето: gap-скан ≤3 без новых
   визуалов — не пропускай; но и мега-компонент на >6 фаз под один ролик, где
   ≥2 подряд сцены идут одной картинкой со сменой подписей — верни режиссёру.

   КРИТИК проверяет драфт сценариста ДО режиссёра и ДО озвучки — на этом шаге
   отсев темы стоит только времени, а после озвучки стоил бы ещё и одной из
   пяти дневных попыток Gemini TTS. Вердикт «БРАК ТЕМЫ» (тема без глубины /
   переигранная рамка / дубль по сути с уже вышедшим) — тема сгорает здесь,
   не доходя до режиссёра: останови сдачу и напиши причину в сводке, НЕ
   переигрывай ту же тему. «ПРАВКИ» (слабый/неподтверждённый факт, лишние
   слова, рамка повторяет недавнее) — один круг обратно сценаристу, не
   больше; не сошлось за этот круг — тоже БРАК ТЕМЫ, а не второй заход.

   СЦЕНАРИСТА, КРИТИКА и РЕЖИССЁРА можно делегировать — но ТОЛЬКО через MCP-сервер
   ...
   (критик сценарий обычно не меняет — закрывай тем же `--allow
   episodes/drafts/{slug}.draft.json` на случай пометок; для режиссёра —
   `--allow episodes/{slug}.json`, а если в ЭТОМ заходе он
   ...
   `.claude/skills/animator/catalog-details.md`, новые файлы под
   ...
   ДЕТЕРМИНИРОВАННЫЕ ПРОВЕРКИ ПЕРЕД РЕНДЕРОМ (после озвучки, обязательны —
   не LLM-суждение, вкус и глубину проверил критик ещё до озвучки, здесь
   только геометрия и движение): `cd video && node scripts/check-overlaps.cjs
   {slug}` — измерение реальных прямоугольников текста в DOM; ненулевой код
   выхода = вернуть режиссёру, рендер и review запрещены до зелёного чека.
   Следом `node scripts/check-motion.cjs episodes/{slug}.json
   video/public/episodes/{slug}/meta.json` — ошибка якоря/границы = точный
   cue режиссёру, не исправляй время руками. БЮДЖЕТ: не более ОДНОГО
   отдельного круга режиссёра ради этих дефектов. Не сошлось за этот бюджет —
   ОСТАНОВИСЬ и отдай оператору (кадры + список дефектов), НЕ круги до
   таймаута. Прецедент auto-20260910-052001 (10.09.2026): второй overlap-круг
   стартовал на 2 ч 40 мин прогона и был убит 180-минутной стеной (exit 124),
   ролик не выпущен. Раунды правок — самый дорогой участок; закладывай их в
   бюджет времени, а не гоняй, пока хватает. Визуальную приёмку целиком
   (развитие картинки, вкус, что уносит зритель) больше не делает LLM — её
   делает оператор в Telegram при Approve; найденные там дефекты чинятся тем
   же кругом режиссёра, без пересъёмки озвучки.
