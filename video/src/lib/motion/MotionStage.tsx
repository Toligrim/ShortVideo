...
import type { ActionPreset, EntrancePreset, MotionPlan, Point } from "./types";
...
export const MotionStage: React.FC<{ start: number; end: number; words: Word[]; plan?: MotionPlan; sampleFrame?: number; children: React.ReactNode }> = ({ start, end, words, plan, sampleFrame, children }) => {
  ...
  return <MotionContext.Provider value={{ frame: sampleFrame ?? currentFrame, start, end, plan, cues }}>{children}</MotionContext.Provider>;
};
export const useMotion = () => {
  const ctx = useContext(MotionContext);
  if (!ctx) throw new Error("MotionGroup/useMotion requires MotionStage");
  const cue = (id: string, fraction = id === "act" ? 0.3 : id === "resolve" ? 0.7 : 0.5): number => ctx.cues[id] ?? Math.round(ctx.start + (ctx.end - ctx.start - 1) * fraction);
  const action = (id: string, fraction = id === "act" ? 0.3 : id === "resolve" ? 0.7 : 0.5): number => {
...
export const MotionGroup: React.FC<{
  ...
  action?: { preset: ActionPreset; cue: string; from?: Point; to?: Point };
  ...
  return <div data-motion-actor={id} data-motion-x={act.x} data-motion-y={act.y} data-motion-progress={action ? motion.action(action.cue) : undefined}
