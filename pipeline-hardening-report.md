...
> **Примечание оркестратора (Claude Code) о происхождении этого файла.**
> Этот отчёт написан НЕ мной, а той самой codex-сессией интеграционного
> аудита, которую я после 30 минут отсутствия ответа от MCP посчитал
> завершившейся неудачей и написал вместо неё собственную версию (см. `git
> log -- pipeline-hardening-report.md`). Как оказалось, процесс на хосте
> продолжал работать и после того, как MCP-клиент перестал ждать ответ, и
> примерно через 12 минут сам закоммитил эту версию прямо в ROOT поверх моей
> — без какой-либо координации со мной в этот момент. Я сверил содержимое:
> эта версия объективно более тщательная (нашла 5 реальных багов в разделе
> 13, которые я пропустил), поэтому оставляю её как основной текст, а не
> переписываю заново. Но два момента стоит знать явно:
> 1. **Процессная гонка.** MCP idle-timeout — это таймаут ожидания ответа у
>    клиента, не гарантия, что сам процесс на хосте остановился. Если такой
>    процесс работает с `cwd=ROOT` (не в изолированном delegate-worktree, как
>    было у этой аудиторской сессии по замыслу самой задачи), он может
>    закоммитить в реальную ветку в любой момент после того, как оркестратор
>    уже решил, что сессия мертва, и начал делать что-то своё в том же
>    дереве. Здесь коллизия свелась к перезаписи одного текстового отчёта и
>    обошлась без потери данных, но при менее удачном стечении обстоятельств
>    (два процесса правят один и тот же код-файл) это мог быть настоящий
>    конфликт. Практический вывод — не начинать читать это как «инцидент
>    предотвращён случайно», а держать в уме на будущее: время ожидания MCP
>    таймаута и время жизни фонового процесса на хосте не гарантированно
>    совпадают.
> 2. **Near-miss в разделе 12.** Там прямо описано: собственный cleanup-скрипт
>    аудитора попытался удалить `root.parent` временного каталога, что
>    оказалось `/tmp`, и остановился только из-за `PermissionError` внутри
>    systemd-private каталога — то есть по счастливой случайности прав
>    доступа, а не потому что код сам себя удержал. Ничего не было утеряно,
>    но это не «сработала защита», а «повезло с правами». Отмечаю явно, а не
>    даю этому потеряться в общем тексте раздела 12.
>
> Ниже — R2→Backblaze B2 миграция (сделана мной, оркестратором, уже после
> того как оба отчёта — мой и этот — были написаны; ни один из них её не
> знает) добавлена отдельным разделом 14 в конец файла.
...
## 14. R2 → Backblaze B2 (сделано позже, оркестратором напрямую)

Cloudflare R2 требует привязку банковской карты даже на бесплатном тарифе —
оператор от этого отказался. `tools/publishing/adapters/r2.py` обобщён под
произвольный S3-совместимый провайдер:

- новый опциональный `SHORTVIDEO_R2_ENDPOINT_URL` (+ `SHORTVIDEO_R2_REGION`,
  дефолт `"auto"`) — при заданном URL `account_id` больше не обязателен и не
  проверяется на 32-hex Cloudflare-формат; endpoint берётся напрямую;
- host-safety проверка подписанного URL (`_validate_signed_url`) обобщена —
  сверяется с хостом реально настроенного `endpoint_url`, а не с хардкодом
  `r2.cloudflarestorage.com`;
- без `SHORTVIDEO_R2_ENDPOINT_URL` поведение полностью прежнее — все
  довесённые ранее R2-тесты прошли без правок;
- doctor (`live.py`) синхронизирован: новый reason-код
  `r2_endpoint_url_invalid`, `_collect_r2_issues` больше не требует
  `SHORTVIDEO_R2_ACCOUNT_ID`, когда задан override;
- добавлено 6 тестов (`tools/test_publishing_r2.py`,
  `tools/test_publishing_instagram.py`), весь `tools/` — 302 passed, 5 skipped
  (те же, что и раньше).

Коммит: `f408eeb49e25a075b703779b6b94782e0ea568a3`.

Backblaze B2 выбран как замена: подтверждённо не требует карты на бесплатном
тарифе (10 ГБ, egress до 3x объёма хранения), S3-совместимый API. Реальные B2
credentials оператору только предстоит завести и вписать в
`SHORTVIDEO_R2_ENDPOINT_URL` / `SHORTVIDEO_R2_REGION` /
`SHORTVIDEO_R2_BUCKET` / `SHORTVIDEO_R2_ACCESS_KEY_ID` /
`SHORTVIDEO_R2_SECRET_ACCESS_KEY` — это по-прежнему operator action, не
сделано за оператора.
